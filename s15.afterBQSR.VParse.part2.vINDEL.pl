...
my $boot = 3;
my $dir_in = "03_GATK";
my @fvcf = File::Find::Rule->maxdepth(1)->file->name(qr/a501-[0-9]+_.*.rnd$boot.$type.flt.vcf$/)->in($dir_in);
...
sub pos2pileup
{
  ...
  while(my $line = <FH_IN>)
  {
    ...
    if($#tmp>=8)
    {
      ...
      # for(my $k=8; $k<=$#tmp; $k++)
      # {
      #   my ($nt,$fcnt,$rcnt) = $tmp[$k] =~ /^([^\(\)]+)\(([0-9]+):([0-9]+)\)/;
      #   $map_cnt{$nt}{'f'} = $fcnt;
      #   $map_cnt{$nt}{'r'} = $rcnt;
      #   $map_cnt{$nt}{'all'} = $fcnt+$rcnt;
      #   $all_cnt += $fcnt+$rcnt;
      # }
      # my ($nt_max) = sort{$map_cnt{$b}{'all'}<=>$map_cnt{$a}{'all'}} keys %map_cnt;
      # my $max_cnt = $map_cnt{$nt_max}{'all'};
      # my $max_fcnt = $map_cnt{$nt_max}{'f'};
      # my $max_rcnt = $map_cnt{$nt_max}{'r'};
      # my $flg = ($all_cnt>0 && nearest(0.01,$max_cnt/$all_cnt)>=0.8 && $max_fcnt>=6 && $max_rcnt>=6 && $max_cnt>=20)? "*":"";
      # $pos2pileup{$tmp[0]}{$tmp[1]} = $flg."$nt_max|".join("|",@tmp[8..$#tmp]);
...
sub vinfo_extractor
{
  ...
  while(my $line = <FH_IN>)
  {
    ...
    if($flg == 1 && $line !~ /^#CHROM/)
    {
      ...
      # $vinfo{$chr}{$pos}{"$ref -> $alt"} = "$info||$fmt||$stat" if $filt eq "PASS" && $dp>=10 && $ref_pl>=0 && $alt_pl==0 && nearest(0.01,$alt_ad/($ref_ad+$alt_ad))>=0.99; # Long GBE 2016
