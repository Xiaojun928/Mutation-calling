...
my $gbff = "../Thermococcus_eurythermalis_A501.new.gbff"; # use the updated reference genome
my %ginfo = %{ginfo_extractor($gbff)};

# prepare kaks_calculator input
my $dir = "..";
my @aa = File::Find::Rule->maxdepth(1)->file->name(qr/new.gene/)->in($dir); # use the updated reference genome

open(FH_OUT,">","nt-aln.axt") or die "Can't open nt-aln.fna: $!";
foreach my $aa (sort @aa)
{
  my $fasta_in = Bio::SeqIO->new(-file=>"$aa",-format=>"fasta");
