...
sub pos2pileup
{
  ...
  while(my $line = <FH_IN>)
  {
    ...
    # {
    #   ($map_cnt{$nt}{'f'}) = join("",@tmp[4..7]) =~ /$nt\(([0-9]+):[0-9]+\)/;
    #   ($map_cnt{$nt}{'r'}) = join("",@tmp[4..7]) =~ /$nt\([0-9]+:([0-9]+)\)/;
    #   $map_cnt{$nt}{'all'} = $map_cnt{$nt}{'f'} + $map_cnt{$nt}{'r'};
    #   $all_cnt += $map_cnt{$nt}{'all'};
    # }
    # my ($nt_max) = sort{$map_cnt{$b}{'all'}<=>$map_cnt{$a}{'all'}} keys %map_cnt;
    # my $max_cnt = $map_cnt{$nt_max}{'all'};
    # my $flg = ($all_cnt>0 && nearest(0.01,$max_cnt/$all_cnt)>=0.99)? "*":"";
    # $pos2pileup{$tmp[0]}{$tmp[1]} = "$flg$index|$nt_max|".join("|",@tmp[4..7]);
