...
my $cnt_dir = "03_GATK";
my $ref_size = 2126164;
my @cnt = File::Find::Rule->maxdepth(1)->file->name(qr/a501-[0-9]+_combined.clean.sort.dedup.rnd3.cnt/)->in($cnt_dir);
my %err = ();
  ...
  while(my $line = <CNT_IN>)
  {
    ...
    if($all_cnt>0 && ($focal_max_fcnt+$focal_max_rcnt)/$all_cnt>=0.8 && $focal_max_fcnt>=3 && $focal_max_rcnt>=3)
    {
      ...
      if($focal_max eq $ref)
      {
        ...
        {
          if($focal_snp ne $focal_max)
          {
            $err{$focal_snp} += $focal_snp{$focal_snp}{'f'} if exists $focal_snp{$focal_snp}{'f'};
            $err{$focal_snp} += $focal_snp{$focal_snp}{'r'} if exists $focal_snp{$focal_snp}{'r'};
          }
          $err{'all'} += $focal_snp{$focal_snp}{'f'} if exists $focal_snp{$focal_snp}{'f'};
          $err{'all'} += $focal_snp{$focal_snp}{'r'} if exists $focal_snp{$focal_snp}{'r'};
        }
      }
    }
...
# error rate
my $ErrA = (exists $err{'A'})? $err{'A'}:0;
my $ErrC = (exists $err{'C'})? $err{'C'}:0;
my $ErrG = (exists $err{'G'})? $err{'G'}:0;
my $ErrT = (exists $err{'T'})? $err{'T'}:0;
my $All = $err{'all'};
open(ERR_OUT,">","ErrorRate.tab") or die "Can't open ErrorRate.tab: $!";
print ERR_OUT "ErrA = $ErrA\t". nearest(0.0001,100*$ErrA/$All) ."%\n";
print ERR_OUT "ErrC = $ErrC\t". nearest(0.0001,100*$ErrC/$All) ."%\n";
print ERR_OUT "ErrG = $ErrG\t". nearest(0.0001,100*$ErrG/$All) ."%\n";
print ERR_OUT "ErrT = $ErrG\t". nearest(0.0001,100*$ErrT/$All) ."%\n";
